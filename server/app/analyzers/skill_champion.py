...
class AnalyzerSkillChampion(AnalyzerBase):
    ...
    def analyze_player_as_an_enemy(self, senpai, player):
        ...
        if stats is None:
            senpai.add_advice(senpai.PROS, EnemyNeverPlayedThisChampionAdvice(player.champion.name))
        elif stats['kda'] < 1:
            senpai.add_advice(senpai.PROS, EnemyHasAGoodKDAWithThisChampionAdvice(player.champion.name, stats['kda']))
        elif stats['played'] > 3 and stats['percent_win'] > 70:
            senpai.add_advice(senpai.CONS, EnemyGoodWithThisChampionAdvice(player.champion.name, stats['percent_win'], stats['pentaKills']))
